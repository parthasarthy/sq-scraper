...
async function checkDate({ from, to, date, cabinClass, passengers, context }) {
  ...
  try {
    ...
    if (/maintenance|challenge/i.test(title)) {
      throw new Error('SQ is showing a bot-challenge page. Re-export cookies: node scripts/export-cookies.js');
    }

    // ── 2. Click Redeem Flights ───────────────────────────────────────────
    await waitAndClick(page, [
      'button:has-text("Redeem flights")',
      'a:has-text("Redeem flights")',
      'label:has-text("Redeem flights")',
      '[data-tab="redeem"]',
      'li:has-text("Redeem flights")',
      'span:has-text("Redeem flights")',
    ]);
